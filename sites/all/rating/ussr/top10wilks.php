...
while($athlete = mysql_fetch_assoc($sql_athlete)) {
  ...
  while($athlete_mwilks = mysql_fetch_assoc($sql_mwilks)) { 
    ...
    print '<td><strong>' . $sql_code['abbreviated_name'] . '</strong></td>'; 
    }
    print '<td><strong>' . $athlete_mwilks['wilks'] . '</strong></td>';
    print '<td>' . $athlete_mwilks['total'] . '/' . $athlete_mwilks['weight'] . '</td>';
    echo '<td class="sm">' . str_replace('(Взрослые)', '', $athlete_mwilks['comp']) . ' ' . $athlete_mwilks['date'] .  '</td>';
  }
  print '</tr>';
}
print '</table>';
...
print '<tr><td>';
	output_top(1, 2, $country);
print '</td>';
print '<td>';
	output_top(2, 2, $country);
print '</td></tr>';
print "<tr><td colspan='2'>{$AdSenseAdaptive}</td></tr>";
print '<tr><td>';
	output_top(1, 1, $country);
print '</td>';
print '<td>';
	output_top(2, 1, $country);
print '</td></tr>';
print '</table>';
?>