...
if (isset($_REQUEST['doAdd']))  
  {
  // подсчитать АТЛЕТОВ с этими данными
  $count['athlete'] = mysql_num_rows(mysql_query(
    "SELECT * FROM " . TABLEATHLETE . " 
      WHERE name = '{$_GET['name_get']}'
  ...
  if (0 < $count['athlete'])
  {
    ...
    $flag_control = $DATABASE->selectRow('SELECT * FROM '.TABLEATHLETE.' 
    WHERE name=? 
    AND age=? 
    AND country=?
    ', $_REQUEST['name_get'], $_GET['age'], $_GET['country']);
    
    // ищет совподения "название соревнований" и "дату соревнований" у добовляемого спортсмена
    //if ($flag_control) 
    {
      ...
      AND devizion=? 
      AND date=?', 
      $flag_control['athlete_id'], $_GET['competition'], $_GET['age_category'], 
      $_GET['devizion'], $_GET['date']);
    }
    
    // подсчитать
    $count['competition'] = $DATABASE->selectCell('
      SELECT COUNT(id) FROM '.TABLECOMPETITION.' 
      WHERE athlete_id=? 
      AND competition=? 
      AND age_category=? 
      AND devizion=? 
      ...
      $flag_control['athlete_id'], $_GET['competition'], $_GET['age_category'], 
      $_GET['devizion'], $_GET['date']);
      //echo $count['competition'];
    

    if (0 < $count['competition'])
    { 
      echo "У спортсмена {$flag_control['name']}, есть в базе данных соревнования 
      \"{$flag_competition['competition']} {$flag_competition['date']}\"";
      echo "<p />Строка совпадения: <u>{$flag_control['competition']} {$flag_control['age_category']} {$flag_control['devizion']} {$flag_control['date']}</u>";
      
      
      // отладка
      print '<pre>Данные переменной <b>flag_control</b><br>';
      print_r($flag_control);
      
      print '<pre>Данные переменной <b>flag_competition</b><br>';
      print_r($flag_competition);
      exit();
    }
    // если спортсмен есть в базе, то добовляем данные только в таблицу _competition
    if ($_REQUEST['name_get'] == $flag_control['name'] AND $_REQUEST['age'] == $flag_control['age'])
    {
      //выбирает из базы ячейку athlete_id для совпавшего имени
      $sql_athlete_id  = $DATABASE->selectRow('SELECT athlete_id, subject_rf FROM '.TABLEATHLETE.' 
                                               WHERE name=? and age=?', $_REQUEST['name_get'], $_REQUEST['age']);
      ...
      $DATABASE->query('INSERT INTO '.TABLECOMPETITION.' SET 
                        id=?, athlete_id=?, devizion=?, date=?, competition=?, position=?, 
                        ...
                        $weight, $squat, $brench, $deadlift, $total, $wilks, $trainer, $age_category);
      Header("Location: 2?".time()).'#textarea';
      exit();
    }
  } else {
