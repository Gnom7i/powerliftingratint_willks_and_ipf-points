<?php
# Файл используется для: частей света. 
...
while($athlete = mysql_fetch_assoc($sql_athlete)) {
  ...
  while($athlete_mwilks = mysql_fetch_assoc($sql_mwilks)) { 
    ...
    echo "<td><a href='/athlete/{$athlete_mwilks['athlete_id']}'>" . get_in_translate_to_en(htmlspecialchars($athlete_mwilks['name'], ENT_QUOTES, 'UTF-8')) . '</a></td>';
...
echo '<tr><th>Women</th>
<td>', av_wilks(1, 2, $part_world), '</td><td>'.av_wilks(1, 1, $part_world).'</td></tr>'."\n";
echo '<tr><th>Men</th>
<td>', av_wilks(2, 2, $part_world), '</td><td>'.av_wilks(2, 1, $part_world).'</td></tr>'."\n";
print '</table>'."\n";
...
echo output_top(1, 2, $part_world)."\n";
echo output_top(2, 2, $part_world)."\n";
echo output_top(1, 1, $part_world)."\n";
echo output_top(2, 1, $part_world)."\n";
print '</table>';
?>